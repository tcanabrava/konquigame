...
const BalloonScene: PackedScene = preload("uid://djfjgwt1guy5f")
const LightsOutDialogue: DialogueResource = preload("uid://qdywlyykljc6")

func _on_dialogue_ended(resource: DialogueResource):
	# Only react to our own dialogue finishing, not any dialogue in the game.
	if resource != LightsOutDialogue:
		return

	canvas_modulate.color = "#111111"

func turn_off_light():
	DialogueManager.show_dialogue_balloon_scene(BalloonScene, LightsOutDialogue, "start")
